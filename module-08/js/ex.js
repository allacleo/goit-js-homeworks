...
const createQuizTest = function(form, quizData) {
  ...
  quizData.questions.map((currentArrayQuestions, currentQuestions) => {
    ...
    sectionTitle.textContent = `${currentQuestions + 1}. ${
      currentArrayQuestions.question
    }`;
    ...
    currentArrayQuestions.choices.map((questions, currentAnswer) => {
...
function handlerSubmitForm(event) {
  ...
  (countCorrectAnswer * 100) / questionsArray.length >= 80
    ? (answer.textContent = 'Ты справился!')
    : (answer.textContent = 'Ты потерпел неудачу:(');
  form.append(answer);
}
