...
const createListItems = ({url, alt}) => {
  const listItems = document.createElement('li');
  const listItemsImg = document.createElement('img');
  
  listItemsImg.setAttribute('url', url);
  listItemsImg.setAttribute('alt', alt);
  listItems.appendChild(listItemsImg);
  
  return listItems;
};
const listItems = galleryItems.map(item => createListItems(item));
